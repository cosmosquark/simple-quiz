...
class SittingAPI(APIView):
    ...
    def get(self, request, pk=None):
        """
        Get the quiz sitting for the current user. If it does not exist,
        then create it.
        """
        if pk is None:
            return Response(serializer.errors,
                            status=status.HTTP_400_BAD_REQUEST)
        sit, created = Sitting.objects.get_or_create(user=request.user,
                                                     quiz_id=pk)
    ...
    def post(self, request, pk=None):
        """
        Post answers in response to a quiz, where pk is the quiz id
        which the user is answering
        """
        if pk is None:
        ...
        if "user_choices" not in request.data:
                return Response({'response': 'Missing User Choices'},
                                status=status.HTTP_400_BAD_REQUEST)
